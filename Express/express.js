...
function valid_user(params, res, connection, uid){
    ...
    pool.getConnection(function(dberr, connection) {
            ...
            app.route('/players/:team_id').get(function(req, res){
                ...
                if(req.params.team_id.match(/[0-9]+/)){
                    ...
                    console.log("select concat(first_name, ' ', last_name) as name, FK_position as  position, player_id as id, signed_date, count(goal_id) as goals  from player left join goal g on g.FK_player_id=player.player_id left join team t on t.team_id=player.FK_team_id where t.name='" + req.params.team_id + "'");
                        connection.query("select concat(first_name, ' ', last_name) as name, FK_position as  position, player_id as id, signed_date, count(goal_id) as goals  from player left join goal g on g.FK_player_id=player.player_id left join team t on t.team_id=player.FK_team_id where t.name='" + req.params.team_id + "'", function(err, result){
                            if(result.length>0){
